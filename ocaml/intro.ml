...
let rec fold_btree f e t = 
  match t with
  | BLf -> e
  | BNode (x, l, r) -> f x (fold_btree f e l) (fold_btree f e r)


let tree_size t = fold_btree (fun _ n m -> 1 + n + m) 0 t

let tree_sum t  = fold_btree (fun x n m -> x + n + m) 0 t

let tree_preorder t = fold_btree (fun x l r -> [x] @ l @ r) [] t
let tree_inorder t = fold_btree (fun x l r -> l @ [x] @ r) [] t
let tree_postorder t = fold_btree (fun x l r -> l @ r @ [x]) [] t

let tree_max_depth t = fold_btree (fun _ n m -> 1 + max n m) 0 t
